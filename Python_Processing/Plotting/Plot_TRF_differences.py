...
power_1 = Extract_TFR(TRF_dir=root_spectrograms, Cond=Condition_1, Class=Class_1, TFR_method=TRF_method,
                      TRF_type=TRF_type)

# Load Class and Condition
power_2 = Extract_TFR(TRF_dir=root_spectrograms, Cond=Condition_2, Class=Class_2, TFR_method=TRF_method,
                      TRF_type=TRF_type)
...
if normalized_bool:
    ...
    # Plot limit per band - [min_lim, max_lim]
    vlim = [(-7.5e-9, 7.5e-9), (-9e-10, 9e-10), (-3e-10, 3e-10), (-9e-11, 9e-11), (-1.2e-10, 1.2e-10),
            (-1.2e-10, 1.2e-10)]
...
for band in range(len(bands)):
    ...
    if save_bool:
        ...
        if baseline_bool:
            fig.savefig(
                save_dir + prefix + '_' + Condition_1 + '_' + Class_1 + '_vs_' + Condition_2 + '_' + Class_2 + '_band' + str(
                    band) + '_Baseline.png',
